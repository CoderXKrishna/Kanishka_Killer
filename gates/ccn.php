...
if (preg_match('/^(\/oyo|\.oyo|!oyo)/', $text)) {
...
//====ANTISPAM AND WRONG FORMAT====//
    if (strlen($message) <= 4) {
            sendMessage($chatId, '<b>• Wrong Format! ⚠️</b>%0A• 𝘚𝘦𝘯𝘥 <code>/chk cc|mm|yy|cvv</code>%0A• 𝘎𝘢𝘵𝘦𝘸𝘢𝘺 <code>Stripe Dead 1 USD</code>', $message_id);
            exit();
  }
$r = "0";
...
━━━━━━━━━━━━━━━━━━
[↯] 𝗣𝗥𝗢𝗫𝗬 ↯ <code>$r.XXX.XXX.XX Dead ❌</code>
...
file_put_contents('./ccs/Deadd.txt', $resp . PHP_EOL, FILE_APPEND);
}
